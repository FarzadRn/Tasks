...
//findZ finds path blocks : a block that touches S||E or Z and <2 spaces
//usage:findZ(grid); grid => grid[][];
void findZ(char grid[ROW][COLUMN])
{
...
void findX(char grid[ROW][COLUMN])
{
	bool wasEffectiveCycle = false;
	char space = char(32);
	char mazeWall = '*';
		...
		for (int i = 0; i < ROW; i++)
		{
			for (int j = 0; j < COLUMN; j++)
			{
				...
				if (grid[i][j] == space && ((i - 1 >= 0) && (j + 1 < COLUMN) && (i + 1 < ROW) && (j - 1 >= 0)))
				{
					...
					if ((up == mazeWall && right == mazeWall) && ((left == space || left == X) && (down == space || down == X)))
					{
						grid[i][j] = X;
						wasEffectiveCycle = true;
					}
					else if ((up == mazeWall && left == mazeWall) && ((right == space || right == X) && (down == space || down == X)))
					{
						grid[i][j] = X;
						wasEffectiveCycle = true;
					}
					else if ((down == mazeWall && left == mazeWall) && ((up == space || up == X) && (right == space || right == X)))
					{
						grid[i][j] = X;
						wasEffectiveCycle = true;
					}
					else if ((down == mazeWall && right == mazeWall) && ((up == space || up == X) && (left == space || left == X)))
					{
						grid[i][j] = X;
						wasEffectiveCycle = true;
					}
...
void findY(char grid[ROW][COLUMN])
{
	bool wasEffectiveCycle = false;
	char space = char(32);
...
void findW(char grid[ROW][COLUMN])
{
	bool wasEffectiveCycle = false;
	char space = char(32);
